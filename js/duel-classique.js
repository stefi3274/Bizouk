/* BiZouk — duel Mode Classique : mêmes mots, message mystère à révéler, on compare le temps */
(function () {
  ...
  const API = {
    async creer(info) {
      ...
      while (essais < 6 && !cree) {
        code = genererCode();
        const { data, error } = await base.from("duels_classique").insert({
          entreprise_id: ent,
          code: code,
    ...
    async lire(code) {
      const base = await db();
      if (!base) return null;
      const { data } = await base.from("duels_classique").select("*")
    ...
    async repondre(code, info) {
      const base = await db();
      if (!base) return null;
      const { data, error } = await base.from("duels_classique").update({
    ...
    lien(code) { return location.origin + "/duel-classique.html?code=" + code; }
  };

  window.BiZoukDuelClassique = API;
})();
